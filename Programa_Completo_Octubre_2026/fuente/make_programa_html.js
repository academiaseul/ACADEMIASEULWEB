...
const fechas = (dia, L) => Array.from({ length: 8 }, (_, w) => { const d = new Date(Date.UTC(2026, 9, 12) + (w * 7 + DIAS[dia] + (dia === "Lunes" ? 7 : 0)) * 86400000); return L === "es" ? `${DIA_ABR.es[dia]} ${d.getUTCDate()} ${MES.es[d.getUTCMonth()]}` : `${DIA_ABR.en[dia]} ${MES.en[d.getUTCMonth()]} ${d.getUTCDate()}`; });
const semanaRango = (w, L) => { const b = new Date(Date.UTC(2026, 9, 12) + w * 7 * 86400000), f = new Date(b.getTime() + 4 * 86400000); return L === "es" ? `${b.getUTCDate()} ${MES.es[b.getUTCMonth()]} – ${f.getUTCDate()} ${MES.es[f.getUTCMonth()]}` : `${MES.en[b.getUTCMonth()]} ${b.getUTCDate()} – ${MES.en[f.getUTCMonth()]} ${f.getUTCDate()}`; };
...
function build(L, cursos, part) {
  ...
  o.push(h1(t.secciones[2]), p(t.escaleraIntro), table(t.escaleraCols, ORDEN.map((id) => [CODIGO[id], `${NOMBRE[L][id]} · ${SUB[L][id]}`, id === "ninos" ? "8–15" : D.cursoPorId(id).cefr, REQ[L][id], horarioCurso(id, L).replace(" · " + chileTxt[L], ""), D.profeDe(D.CLASES.find((c) => c.cursoId === id).profeId).corto, String(cupo(id))])), p(t.ninosNota), p(t.conv2));
  const f = { Lunes: fechas("Lunes", L), Martes: fechas("Martes", L), "Miércoles": fechas("Miércoles", L), Jueves: fechas("Jueves", L) };
  o.push(h1(t.secciones[3]), p(t.calIntro), table(t.calCols, Array.from({ length: 8 }, (_, w) => [String(w + 1), semanaRango(w, L), f.Lunes[w] + (w === 0 ? " *" : ""), f.Martes[w], f.Martes[w], f["Miércoles"][w], f.Jueves[w], f.Jueves[w]])), p("* " + t.feriadoNota), h3(L === "es" ? "Hitos" : "Milestones"), kv(t.hitos));
  ...
  for (const id of ORDEN) {
    ...
    o.push(h2(t.ficha), kv([[t.fichaRows[0], CODIGO[id]], [t.fichaRows[1], id === "ninos" ? (L === "es" ? "Niños de 8 a 15 años" : "Kids aged 8 to 15") : "CEFR " + D.cursoPorId(id).cefr], [t.fichaRows[2], horarioCurso(id, L)], [t.fichaRows[3], D.profeDe(clases[0].profeId).nombre], [t.fichaRows[4], c.requisito_entrada || REQ[L][id]], [t.fichaRows[5], String(cupo(id)) + (clases.length > 1 ? (L === "es" ? " por sección" : " per section") : "")], [t.fichaRows[6], D.cursoPorId(id).libro], [t.fichaRows[7], dias.map((d) => fechas(d, L)[0] + " → " + fechas(d, L)[7]).join(" · ")]]));
