      ...
      {PROFES.map((p) => {
        ...
        return (
          ...
          >
            ...
            {!compacto && <p className="text-sm text-gray-600">{p.bio}</p>}
            {activo && <div className="mt-2 text-[10px] font-bold tracking-widest uppercase px-2 py-0.5 rounded-full bg-[#E8B84B] text-[#0D0D0D] inline-block">tu profe</div>}
          </div>
        );
      })}
    </div>
