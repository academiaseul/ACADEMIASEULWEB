...
export default function FaqContent() {
  ...
  const whatsappUrl =
    ...
    encodeURIComponent(t("Hola Jay! Vi academiaseul.com y tengo una duda sobre los cursos de coreano."));

  // JSON-LD se mantiene en español (datos estructurados para buscadores).
  const jsonLd = {
    ...
    <main className="min-h-screen bg-[#F4F7FF]">
      ...
      <section className="pt-32 md:pt-40 pb-12 px-6 text-center">
        <p className="text-seoul-red text-xs font-bold tracking-[4px] uppercase mb-4">
          {t("질문 · Preguntas frecuentes")}
        </p>
        <h1 className="text-4xl md:text-6xl font-black text-seoul-black mb-4">
          {t("Todo lo que")} <span className="text-seoul-red">{t("quieres saber")}</span>
        </h1>
        <p className="text-gray-600 max-w-xl mx-auto">
          {t("Si después de leer esto te queda alguna duda, escríbenos: te respondemos personalmente.")}
        </p>
      </section>
      ...
      <section className="text-center pb-24 px-6">
        <p className="text-gray-500 text-sm mb-4">{t("¿Tienes otra duda?")}</p>
        <a
          href={whatsappUrl}
            target="_blank"
