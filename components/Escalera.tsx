...
const ENTRADAS: { key: Entrada; label: string; cursoId: CursoId; frase: string }[] = [
  { key: "cero", label: "Nunca estudié coreano", cursoId: "a11", frase: "Tu curso es Básico 1 (A1.1): empiezas leyendo el alfabeto y terminas presentándote." },
  { key: "hangul", label: "Ya leo Hangul", cursoId: "a12", frase: "Si ya lees 한글 y te presentas, tu curso es Básico 2 (A1.2): del presente al pasado y al futuro." },
  { key: "nivel1", label: "Terminé el Nivel 1 de julio", cursoId: "a12", frase: "¿Fuiste alumno/a del Nivel 1 en julio? Ese curso hoy se llama Básico 1 (A1.1). Tu siguiente paso es Básico 2 (A1.2) · miércoles 21:00 Chile · con Jay." },
  { key: "hablar", label: "Quiero hablar", cursoId: "a21", frase: "Con Básico 2 terminado (o test de nivel), Conversacional 1 (A2.1) con la Prof.ª Abby: puro hablar con una nativa." },
  { key: "topik", label: "Voy por el TOPIK", cursoId: "topik2", frase: "Grupo de máximo 8 enfocado 100% en el examen oficial: TOPIK II (B1+) · jueves 21:00 Chile · con Jay." },
  { key: "ninos", label: "Es para mi hijo/a", cursoId: "ninos", frase: "Coreano para Niños (8–12): lunes 18:00 Chile, juegos, canciones y un show final para la familia." },
];
...
export default function Escalera({
  ...
  const peldanos: Peldano[] = [
    ...adultos.map((c) => ({
      ...
      cefr: c.cefr,
      sub: `${horarioDe(c.cursoId, pais)}${pais !== "chile" ? ` ${info.corto}` : ""} · ${profeCorto(clasesDe(c.cursoId)[0])}`,
      futuro: false,
      href: `/nivel-1?clase=${primeraClaseDe(c.cursoId)}#clases`,
      paso: c.paso,
    })),
    { key: "a22", emoji: "🎎", nombreCorto: CONVERSACIONAL_2.nombreCorto, cefr: CONVERSACIONAL_2.cefr, sub: `Abre en ${CONVERSACIONAL_2.cuando}`, futuro: true, href: CONVERSACIONAL_2.href, paso: CONVERSACIONAL_2.paso },
  ...
  return (
    <div className="w-full">
      {conChips && (
        <div className="mb-5">
          <div className={`text-xs font-bold tracking-widest uppercase mb-2 ${sub}`}>¿Cuál es tu caso?</div>
          ...
          {sel && (
            <div className={`mt-3 rounded-2xl border-2 border-[#4236F6] p-4 flex flex-col sm:flex-row sm:items-center gap-3 ${oscuro ? "bg-white/5" : "bg-[#F5F3FF]"}`}>
              <p className={`flex-1 text-sm leading-relaxed ${oscuro ? "text-white" : "text-gray-800"}`}>{sel.frase}</p>
              <a
              ...
              >
                {sel.cursoId === "ninos" ? "Inscribir a mi hijo/a →" : `Ir a ${CURSOS.find((c) => c.cursoId === sel.cursoId)!.nombreCorto.split(" (")[0]} →`}
              </a>
            </div>
          )}
        </div>
              ...
              >
                <div className="flex items-center gap-3 md:block">
                  ...
                  <div className="min-w-0">
                    <div className="font-bold leading-tight text-sm md:text-[15px]">{p.nombreCorto}</div>
                ...
                >
                  {p.futuro ? "enero 2027" : `paso ${p.paso}`}
                </div>
                {esActivo && (
      ...
      <div className="mt-4">
        ...
        >
          ...
          <span>
            <span className="font-bold text-sm md:text-[15px]">{ninos.nombreCorto}</span>
            {!compacta && <span className={`block text-xs ${sub}`}>{horarioDe("ninos", pais)}{pais !== "chile" ? ` ${info.corto}` : ""} · carril propio para niños y niñas</span>}
          </span>
          {resaltado === "ninos" && (
            <span className="absolute -bottom-2.5 left-3 text-[10px] font-bold tracking-widest uppercase px-2 py-0.5 rounded-full bg-[#E8B84B] text-[#0D0D0D]">tu curso</span>
          )}
        </a>
      </div>
    </div>
